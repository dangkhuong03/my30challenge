...
for(const [code,text] of [["payload","content-data.js?v=full-rebuild-v1"],["day-schema","day-data.js?v=day-schema-v1"],["structured-route","window.CHALLENGE_DAYS"],["typed-renderer","function renderBlocks"],["typed-content-router","function renderTypedContent"],["runtime-build-marker","SCHEMA V1 · 30 DAYS"],["runtime-error-surface","id=\"runtimeError\""],["progress","localStorage"],["append-events","events:"],["gating","assessmentLocked"],["status","validStatuses"],["learn-build","id=\"learnBuildBlock\""],["practice-verify","id=\"practiceVerifyBlock\""],["evidence-continue","id=\"evidenceContinueBlock\""],["mathjax-loader","vendor/mathjax/tex-svg.js"],["latex-normalizer","function toLatexMath"],["dynamic-typeset","function typesetMath"],["display-math","math-display-source"]]) html.includes(text)?out.pass(code,text):out.fail(code,`${text} missing`);
...
const declaredIds=(html.match(/const ids = \[([^\]]+)\]/)||[])[1]?.match(/"([^"]+)"/g)?.map(value=>value.slice(1,-1))||[];
const missingDomIds=declaredIds.filter(id=>!html.includes(`id="${id}"`));
missingDomIds.length?out.fail("dom-id-contract",`Missing DOM ids: ${missingDomIds.join(", ")}`):out.pass("dom-id-contract",`${declaredIds.length} runtime DOM references exist`);
